...
open class LPANSIColorLogFormatter: LPLogFormatterProtocol {
    ...
    internal var formatStrings: [LPLogger.Level: String] = [:]
    
    /// Internal cache of the description for each log level
    internal var descriptionStrings: [LPLogger.Level: String] = [:]
    ...
    open func colorize(level: LPLogger.Level, with foregroundColor: ANSIColor = .default, on backgroundColor: ANSIColor = .default, options: [ANSIOption] = []) {
        ...
        formatStrings[level] = LPANSIColorLogFormatter.escape + codes.joined(separator: ";") + "m"
    ...
    open func colorize(level: LPLogger.Level, custom: String) {
        if custom.hasPrefix(LPANSIColorLogFormatter.escape) {
            formatStrings[level] = "\(custom)"
            descriptionStrings[level] = "Custom: \(custom[custom.index(custom.startIndex, offsetBy: LPANSIColorLogFormatter.escape.lengthOfBytes(using: .utf8)) ..< custom.endIndex])"
        }
        else {
            formatStrings[level] = LPANSIColorLogFormatter.escape + "\(custom)"
    ...
    internal func formatString(for level: LPLogger.Level) -> String {
        return formatStrings[level] ?? LPANSIColorLogFormatter.reset
    }
    ...
    open func format(logDetails: inout LPLogDetails, message: inout String) -> String {
        message = "\(formatString(for: logDetails.level))\(message)\(LPANSIColorLogFormatter.reset)"
        return message
    }
}
